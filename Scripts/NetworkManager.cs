...
public class NetworkManager : MonoBehaviour
{
    ...
    //public Chessman selectedChessmanNetwork;
    public Vector3 movPawn;
    public Vector3 secPawn;
    public bool playerTurn = true;
    public Vector3 turnPlayer;
    public Vector3 clockTurn = new Vector3(0, 0 , 0);
    ...
    void Start()
    {
        ...
        socket.On("move piece", OnPlayerMovePawn);
        socket.On("select piece", OnPlayerSelectPawn);
        socket.On("stop clock", OnPlayerStopClock);
        socket.On("stop clockB", OnPlayerStopClockBlack);
    }

    public void JoinGame()
    {
        StartCoroutine(ConnectToServer());
        clockTurn = new Vector3(0, 0, clockTurn.z + 1);
    }
    ...
    public void CommandMovePawn(Vector3 vec3)
    {
        //Debug.Log("moviendo el peon");
        //string data = JsonUtility.ToJson(new PositionJSON(vec2));
        //socket.Emit("move piece", new JSONObject(data));
        //socket.Emit("move piece", )
        string data = JsonUtility.ToJson(new PositionJSON(vec3));
        socket.Emit("move piece", new JSONObject(data));
    }

    public void CommandSelectPawn(Vector3 vec3)
    {
        string data = JsonUtility.ToJson(new PositionJSON(vec3));
        socket.Emit("select piece", new JSONObject(data));
    }

    public void CommandStopClock(Vector3 vec3)
    {
        //Debug.Log(playerTurn);
        string data = JsonUtility.ToJson(new PositionJSON(vec3));
        //playerTurn = !playerTurn;
        //Debug.Log(playerTurn);
        socket.Emit("stop clock", new JSONObject(data));
    }

    public void CommandStopClockBlack(Vector3 vec3)
    {
        //Debug.Log(playerTurn);
        string data = JsonUtility.ToJson(new PositionJSON(vec3));
        //playerTurn = !playerTurn;
        //Debug.Log(playerTurn);
        socket.Emit("stop clockB", new JSONObject(data));
    ...
    public class UserTurn
    {
        public string name;
        public bool turn;
        public static UserTurn CreateFromJSON(string data)
        {
            return JsonUtility.FromJson<UserTurn>(data);
        }
    }
    ...
    public class PawnJSON
    {
        public float x;
        public float y;
        public static PawnJSON CreateFromJSON(string data)
        {
            return JsonUtility.FromJson<PawnJSON>(data);
        }
    }
